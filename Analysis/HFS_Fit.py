...
#    tm_fit = exp(Fm_fit, t0, dF)    
    ...
    sp.set_ylabel('Dwell time (s)')
    sp.set_title("Force vs Dwell time")  
    
    sp = fig.add_subplot(122)   
    sp.plot(Fs, ks, 'ko', ms=10, alpha=0.5)              
    sp.axvline(x=0, c='k', ls='dotted', lw=1)   
    sp.set_ylim(0, max(ks)*1.1)
    sp.set_xlabel('Force (pN)')  
    sp.set_ylabel('Detachment rate (1/s)')
    sp.set_title("Force vs Detachment rate")       
